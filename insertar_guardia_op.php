        ...
        switch ($accion) {
          case 1:  // Accion 1 -- Registro de Solicitud
            ...
            if (isset($_POST)) { // Asignamos valores a las variables del POST
              ...
              $consulta_sector = pg_query($dbconn, "SELECT * FROM sectores WHERE id = $sector_sol");
              $reg_prrq = pg_fetch_array($consulta_sector);
              if ($reg_prrq != 0){
              $parroquias_id = $reg_prrq['parroquias_id'];
            }else{
              $parroquias_id = '00';
            } 
              ...
              if (pg_query($dbconn, $bitacora_obs_pg) or die("Upps Fallo conexion con la Tabla")) {
                ...
                header("Location: operador/principal.php?msg=1");
              } else {

                header("Location: operador/nueva_solicitud.php?msg=3");
              }
            }
...
//            }
          ...
          case 3: // Accion 3 -- Registro de reporte de solicitudes
            ...
            if (isset($_POST)) { // Asignamos valores a las variables del POST
              ...
              if (pg_query($dbconn, $bitacora_obs_pg) or die("Upps Fallo conexion con la Tabla de Reporte de Solicitud")) {
                // redireccionamiento
                header("Location: operador/principal.php?msg=2");
              } else {
                header("Location: operador/solicitud_consulta.php?msg=2");
              }
            }
            /*************************************** 		FIN BREAK 2 	****************************************/
            break;
          case 4: // Accion 4 -- Registro de observaciones de guardia
            ...
            if (isset($_POST)) { // Asignamos valores a las variables del POST
              ...
              if (pg_query($dbconn, $bitacora_obs_pg) or die("Upps Fallo conexion con la Tabla")) {
                // redireccionamiento
                header("Location: operador/principal.php?msg=3");
              } else {
                header("Location: operador/observaciones_personal.php?msg=1");
              }
            }
            /***************************************     FIN BREAK 3   ****************************************/
            break;
